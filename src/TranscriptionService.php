...
class TranscriptionService
{
    ...
    public function formatBookingResult($result, $userLanguage = 'ru')
    {
        ...
        $isRu = ($userLanguage === 'ru');

        // Если не распознана услуга — не начинаем подтверждение, а просим выбрать услугу
        if (!$service) {
            return $isRu
                ? "🎤 Я понял, что вы хотите забронировать визит. Пожалуйста, выберите услугу кнопками выше, затем скажите дату и время (например: ‘завтра в 19:00’) и количество гостей."
                : "🎤 I got that you want to book a visit. Please choose a service from the buttons above, then tell me the date and time (e.g. ‘tomorrow at 7 PM’) and number of guests.";
        }

        // Если не хватает даты/времени — просим только недостающие данные
        $missing = [];
        if (!$date) { $missing[] = $isRu ? 'дату' : 'date'; }
        if (!$time) { $missing[] = $isRu ? 'время' : 'time'; }

        if (!empty($missing)) {
            $need = $isRu ? 'Уточните, пожалуйста' : 'Please specify';
            $and = $isRu ? ' и ' : ' and ';
            $what = implode($and, $missing);
            return $isRu
                ? "📅 **Черновик бронирования:**\n\n🏊‍♀️ **Услуга:** {$service}\n👥 **Гостей:** {$guests}\n\n{$need} {$what}. Пример: ‘завтра в 19:00’."
                : "📅 **Booking draft:**\n\n🏊‍♀️ **Service:** {$service}\n👥 **Guests:** {$guests}\n\n{$need} {$what}. Example: ‘tomorrow at 7 PM’.";
        }

        // Полный набор данных — показываем подтверждение
        if ($isRu) {
